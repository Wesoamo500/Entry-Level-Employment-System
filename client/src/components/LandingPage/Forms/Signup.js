...
const Login = () => {
	...
	const [formData,setFormData] = useState({firstname:'',surname:'',email:'',password:'',companyname:'',location:'',pic:'',school:'',year_completed:''})

	const handleSubmit = (e)=>{
		e.preventDefault()
		dispatch(signUp(formData))
		setFormData({firstname:'',surname:'',email:'',password:'',companyname:'',location:'',pic:'',school:'',year_completed:''})	
	}
		...
		<div className='register_form'>
			...
			<form className='regis-form' method='post' onSubmit={handleSubmit}>
				...
				{change ?
					<>
						<Grid item  xs={12} sm={6}>
							<TextField size='small' required type='text' label='Company Name' name='companyname' onChange={handleChange} fullWidth/>	
						</Grid>
						<Grid item  xs={12} sm={6}>
						...
						<Grid item xs={12}>
							<TextField size='small'  type='date' label='Date Completed' name='year_completed' onChange={handleChanger} fullWidth />
						</Grid>
					</>
				}
				</Grid>
				{
					loading ?  <LoadingButton sx={{ mt: 3, mb: 2 }} loading variant="contained" fullWidth>Submit</LoadingButton> : <Button size='small' type='submit' variant='contained' onSubmit={handleSubmit} fullWidth  sx={{ mt: 3, mb: 2 }}>Register</Button>
                }
				{
                    	error && <Alert variant="contained" severity="error">Signup Failed</Alert>
                }
				
				<Button onChange={handleChange} variant='outlined' onClick={changeSub} fullWidth  >{change?'Register as a graduate':'Register as a recruiter'}</Button>
			</form>
			
			<div className='alt_logo'>
				
			</div>
			<p className='p-addedd'>Already have an account? Login <a href='/login'>Here</a></p>
		</div>
  )
}
